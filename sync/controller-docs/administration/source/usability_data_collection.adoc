...
When you imported your license for the f.adoc[] time, you were given
...
`Details of data collection<usability_data_collection_details>` below.
...
authentication_backends::
  What user authentication backends are available. See xref:ag_social_auth[]
  and xref:ag_auth_ldap[] for details
external_logger_enabled::
  Whether external logging is enaled
external_logger_type::
  What logging backend is in use if enabled. See xref:ag_logging[] for
  details
logging_aggregators::
  What logging categories are sent to external logging. See xref:ag_logging[]
  for details
free_instances::
