...
For instructions, see the `ug_wf_editor` section.
...
inventory. For more information about job slices, see `ug_job_slice`.
...
from this job template. Refer to `ug_provisioning_callbacks` for
...
you want to run job slices simultaneously. Refer to `ug_job_concurrency`
...
....
+
See `ug_jobtemplates_extravars` for more information.
* *Prompt on Launch*: If selected, even if a default value is supplied,
...
example survey created in `ug_surveys`.
...
fact caching. For more information, see `ug_fact_caching`.
