...
Ansible Controller includes three default {ExecEnvShort}s:

* `Ansible 2.9` - no collections are installed other than Controller modules
* `Minimal` - contains the latest Ansible 2.13 release along with {Runner}, but contains no collections or other additional content
...
-----
+
. Configure your {Builder} files to specify the desired base image and any additional content to add to the new {ExecEnvShort} image.
...
[NOTE]
=====
You must have `admin` or appropriate container repository permissions for {HubName} to push a container. See _Managing containers in {PrivateHubName}_ in the link:https://access.redhat.com/documentation/en-us/red_hat_ansible_automation_platform[{PlatformName} documentation] for more information.
=====
+
-----
