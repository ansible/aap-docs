...
+
`mkdir -p collections/ansible_collections/myorg/mcp_cfn/{galaxy.yml,README.md,LICENSE}`
+
`mkdir -p collections/ansible_collections/myorg/mcp_cfn/roles/meta/{main.yml}`
...
+
`cd collections/ansible_collections/myorg/cfn_mcp`
`ansible-galaxy collection build --output-path /path/to/output/`
+
This produces a file `myorg-cfn_mcp-1.0.1.9.tar.gz`.