...
* You have created an API token, as in link:{BaseURL}/red_hat_ansible_automation_platform/{PlatformVers}/html/managing_automation_content/proc-create-api-token[Creating the API token in {HubName}] and stored it in a secure location, for example in a file named `token.txt`.
