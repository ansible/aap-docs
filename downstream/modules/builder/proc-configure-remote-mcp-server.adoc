...
When type is http, the framework skips installation and only registers the server in the manifest with its URL.
...
. Users can then select the mode when building the execution environment:
+
----
