...
= Pushing signed {ExecEnvShort}s from your local environment

[role="_abstract"]
Sign an {ExecEnvNameSing} on a local system and push the signed {ExecEnvShort} to the {HubName} registry.

.Procedure

. From a terminal, log in to Podman, or any container client currently in use, and pull the {ExecEnvShort} you want to sign.
+
----
...
----
+
. After the {ExecEnvShort} has been pushed, navigate to {MenuACExecEnvironments}.
