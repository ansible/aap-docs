...
[role="_abstract"]
You can pull {ExecEnvName} from the {PrivateHubName} remote registry to sync an image to your local machine.
To sync an {ExecEnvShort} from a remote registry, you must first configure a remote registry.
...
Note that syncing {ExecEnvName} with a large number of tags is time consuming and uses a lot of disk space.
