...
This ensures that downloaded collections are approved by your organization and have not been changed after they were uploaded to {PlatformNameShort}.

If a collection has been signed by {PlatformNameShort}, the server provides ASCII armored, GPG-detached signatures to verify the authenticity of `MANIFEST.json` before using it to verify the collection’s contents. 
...
* Signed collections are available in {PlatformNameShort} to verify signature.
...
[NOTE]
====
In addition to any signatures provided by {PlatformNameShort}, signature sources can also be provided in the requirements file and on the command line. 
...
----
+
When you install a collection from {PlatformNameShort}, the signatures provided by the server are saved along with the installed collections to verify the collection's authenticity.
...
[discrete]
= How do I get a namespace on {PlatformNameShort}?

By default namespaces used on {Galaxy} are also used on {PlatformNameShort} by the Ansible partner team. 
