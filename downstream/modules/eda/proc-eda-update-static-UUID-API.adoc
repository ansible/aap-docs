...
[role="_abstract"]
If you prefer to use a static, custom UUID, you must use the API to update your event stream. The API endpoint for creation is `POST /api/eda/v1/event-streams/`.
...
.Verification
*Confirm that the UUID of your event stream has been updated to the new static input.