...
[IMPORTANT] 
====
The `ansible.eda` collection requires specific system dependencies (`libsystemd0`, `libsystemd-dev`, and `pkg-config`) to function correctly. These must be included in your decision environment build as shown in the following examples.
====
+
...
dependencies:
  galaxy:
    collections:
      - ansible.eda
  system:          
      - libsystemd0 [platform:rpm platform:dpkg]                                                            
      - libsystemd-dev [platform:rpm platform:dpkg]                                                         
      - pkg-config [platform:rpm platform:dpkg]
  python_interpreter:
