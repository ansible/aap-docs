...
* For more information about modifying simultaneous rulebook activations during or after {EDAName} on {OCPShort}, see link:{URLOperatorInstallation}/appendix-operator-crs_performance-considerations#eda_max_running_activations[EDA_MAX_RUNNING_ACTIVATIONS]. link:{URLOperatorInstallation}/appendix-operator-crs_performance-considerations#gateway_only_yml[gateway-only.yml]
