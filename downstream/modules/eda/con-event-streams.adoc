...
h| Mutual TLS | Needs the vendor's CA certificate to be present in our Envoy proxy, which can be delivered via {EDAName} credentials at runtime. This supports non-repudiation. | PagerDuty
|===
