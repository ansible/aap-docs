...
. Create your python requirements.txt and ansible collection requirements.yml with the content needed for your custom {ExecEnvShort} image.  Note that any collections you require should already be uploaded into your {PrivateHubName}.
...
----
+
. Create the {ExecEnvShort} image in the local podman cache using the `podman` command.
+
----
