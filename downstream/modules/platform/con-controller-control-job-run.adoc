...
[role="_abstract"]
You can control where jobs run by using instance groups.
By default, when you launch a job, {ControllerName} selects an instance group that has available capacity to run the job.
This default behavior can be overridden by associating instance groups with job templates, inventories, or organizations.

When a job is launched, {ControllerName} checks for any instance groups associated with the job template, inventory, or organization (by way of project) in that order.
...
Jobs must run in those groups in preferential order as resources are available.
