...
[role="_abstract"]
Configure a custom Ansible configuration file (ansible.cfg) by placing it at the project root. 
This allows {ControllerName} to customize settings for job execution.

While Ansible does not require a configuration file, operating system packages often include a default one in `/etc/ansible/ansible.cfg` for possible customization. 
...
//To learn which values you can use in this file, see link:https://docs.ansible.com/projects/ansible/latest/reference_appendices/config.html#generating-a-sample-ansible-cfg-file[Generating a sample ansible.cfg file] in the Ansible documentation.
