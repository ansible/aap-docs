...
* If you configure the `aap_gateway_url` parameter only, the Ansible Lightspeed Service MCP server is provisioned. Authentication will attempt to use the JSON Web Token (JWT) token associated with the user's authenticated context.
* If you configure both parameters `aap_gateway_url` and `aap_controller_url`, the Ansible Lightspeed Service MCP server and {PlatformNameShort} Controller Service MCP server are both configured. Authentication will attempt to use  the JWT token associated with the user's authenticated context.
|====
