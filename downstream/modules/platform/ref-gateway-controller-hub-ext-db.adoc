...
= Single platform gateway, {ControllerName}, and {HubName} with an external (installer managed) database

[role="_abstract"]
Use this example to populate the inventory file to deploy single instances of platform gateway, {ControllerName}, and {HubName} with an external (installer managed) database.
