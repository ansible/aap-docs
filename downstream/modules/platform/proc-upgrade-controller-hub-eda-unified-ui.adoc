...
{PlatformNameShort} 2.5 supports upgrades from legacy 2.4 environments as well as the current version of {EDAName} to {PlatformNameShort} 2.5 with a unified UI using the RPM install method only. Combining install methods (OCP, RPM, Containerized) within such a topology is not supported by {PlatformNameShort}.
