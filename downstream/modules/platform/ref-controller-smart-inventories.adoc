...
[role="_abstract"]
Smart Inventories are collections of hosts defined by a stored search that can be viewed such as a standard inventory and can be easily used with job runs. 
...
The membership table updates every time a job runs against a smart inventory.

[NOTE]
====
To update the memberships more often, you can change the `AWX_REBUILD_SMART_MEMBERSHIP` file-based setting to `True`. (The default is False).
...
The administrator of a Smart Inventory has permission to edit fields such as the name, description, variables, and the ability to delete. 
The administrator does not have the permission to change the `host_filter`, because that affects which hosts (that have a primary membership inside another inventory) are included in the smart inventory. 
