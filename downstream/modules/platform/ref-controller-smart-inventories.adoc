...
Organization administrators have admin permission for inventories in their organization and can create a Smart Inventories. 
...
Administrators of the Smart Inventory can grant other users (who are not also admins of your organization) permissions such as "use" and "adhoc" to the smart inventory. These permit the actions indicate by the role, just like other standard inventories. 
However, this does grant any special permissions to hosts (which live in a different inventory).
It will not permit direct read permission to hosts, or permit them to see additional hosts under `/#/hosts/`, although they can still view
...
include::ref-controller-smart-host-filter.adoc[leveloffset=+1]
include::proc-controller-define-filter-with-facts.adoc[leveloffset=+1]
