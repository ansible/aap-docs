...
A resource can generate an identifier format therefore, have a named URL if it has at least one unique key that satisfies the following rules:
...
. The only allowed exceptional fields that breaks the preceding rule is a many-to-one related field relating to a resource other than itself, which is also allowed to have a slug.
