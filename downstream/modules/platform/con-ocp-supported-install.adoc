...
* If you plan to install {EDAcontroller} on {OCPShort} and modify the number of simultaneous rulebook activations, you must add the `EDA_MAX_RUNNING_ACTIVATIONS` parameter. See the example in appendix link:{URLOperatorInstallation}#eda_max_running_activations[EDA_MAX_RUNNING_ACTIVATIONS]. By default, {EDAcontroller} allows 12 activations per node to run simultaneously.

[NOTE]
====
`EDA_MAX_RUNNING_ACTIVATIONS` for {OCPShort} is a global limit since there is no concept of worker nodes in {OCPShort}.
====
