...
* To calculate the `max_concurrent_jobs` required on a container group consider the `pod_spec` setting for that container group. In the `pod_spec`, you can see the resource requests and limits for the automation job pod. Use the following equation to calculate the maximum concurrent jobs that you need: 
----
...
----
** For example, if your `pod_spec` indicates that a pod will request a 250 mcpu Kubernetes cluster that has 1 worker node with 2 CPU, the maximum number of jobs that you need to start with is 8.
