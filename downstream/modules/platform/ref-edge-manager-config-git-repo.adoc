...
The `Path` directory corresponds to the root directory (`/`) on the device, unless you sepcify the `MountPath` parameter.
...
The `Repository` resource defines the Git repository, the protocol and access credentials that the {RedHatEdge} must use.
You only need to set up the repository once.
After setting up, the you can use the repository to configure individual devices or device fleets.
