...
= Create the custom {ExecEnvShort} definition
...
----
+
. Create a `pip.conf` file under the `files/` subdirectory which points to the internal PyPI mirror (a web server or something similar to Nexus):
+
----
$ cat files/pip.conf
[global]
