...
When the {Builder} RPM is installed, use the following steps to create your custom {ExecEnvShort}.

.Procedure

. Create a directory for the build artifacts used when creating your custom {ExecEnvShort}.	
...
.. Add the `hub-ca.crt` file to the `files/`` subdirectory.
+
. When the preceeding steps have been completed, create your python `requirements.txt` and ansible collection `requirements.yml` files, with the content needed for your custom {ExecEnvShort} image. 
+
