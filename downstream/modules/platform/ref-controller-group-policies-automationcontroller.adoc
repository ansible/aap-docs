...
After you run the installer, you see the following error:
...
After you upgrade from {ControllerName} 4.0 or earlier, the legacy `instance_group_` member most likely has the awx code installed, which causes that node to be placed in the `automationcontroller` group.
