...
= Creating Red Hat Lightspeed credentials
...
To enable integration between {PlatformNameShort} and {RHLightspeed}, assign the service account with the following permissions:
...
Use the following procedure to create a new credential for use with {RHLightspeed}:
