...
Hop nodes serve to route traffic from one part of the automation mesh to another (for example, could be a bastion host into another network). RAM can affect throughput, CPU activity is low. Network bandwidth and latency are generally a more important factor than either RAM or CPU.
