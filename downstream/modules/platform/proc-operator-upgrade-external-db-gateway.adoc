...
To upgrade from {PlatformNameShort} 2.4 to {PlatformVers} with an external database, you must scale down your Operator deployment, upgrade your PostgreSQL, then scale your deployment back up.
...
. Create a secret `postgres-config-gateway` with {PostgresVers} credentials for the {Gateway} component.
For example:
+
----
apiVersion: v1
...
. Select *stable-2.6* as the channel and click btn:[Save].
