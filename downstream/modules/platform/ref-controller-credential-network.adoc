...
If you assign both a Machine credential and a Network credential to the same job template, {ControllerName} injects them independently. The Machine credential controls SSH authentication, while the Network credential sets the `ANSIBLE_NET_USERNAME` and `ANSIBLE_NET_PASSWORD` environment variables.      
                  
For information on combining credential types, see                               xref:proc-controller-multi-cred-machine-network[Using Machine and Network credentials together].  
====
