...
Where `refresh_token` is provided by `refresh_token` field of the access token above that. 
...
Verify that new token is present and the old one is deleted in the `/api/v2/tokens/` endpoint.
