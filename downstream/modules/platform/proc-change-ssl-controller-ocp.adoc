...
= Changing the {ControllerName} SSL/TLS certificate and key on {OCPShort}

[role="_abstract"]
The following procedure describes how to change the SSL/TLS certificate and key for {ControllerName} running on {OCPShort}.
...
----
+
. Edit the {PlatformNameShort} custom resource to add `route_tls_secret` under the `controller` section:
+
----
oc edit ansibleautomationplatform <aap-instance-name> -n <namespace>
----
+
. Add or update the `route_tls_secret` parameter under `spec.controller`:
+
[source,yaml]
----
...
The operator automatically applies this configuration to the {ControllerName} instance managed by this {PlatformNameShort} deployment.
====
