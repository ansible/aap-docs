...
[role="_abstract"]
Scaling guidelines for {EDAName} define how to properly size API nodes based on user requests and worker nodes based on automation activations. Following these guidelines, specifically by separating the node roles, ensures independent scaling and optimizes resource utilization for your environment.
