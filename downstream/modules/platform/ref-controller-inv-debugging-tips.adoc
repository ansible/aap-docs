...
For example, if a `groups` group has a complex filter (such as `shutdown_in_product_dev`) but does not contain any hosts in the resultant constructed inventory, then use the `compose` parameter to help debug. 
