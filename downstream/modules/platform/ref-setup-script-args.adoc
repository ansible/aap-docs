...
When you run the setup.sh script using this argument, by default, a new secret key is generated and distributed for the {ControllerName} but not for the {CatalogName}. 

To generate and distribute a new secret key for both the {ControllerName} and the {CatalogName}, specify the variable `rekey_catalog: true`. 
...
|`rekey_catalog`|When you run the `setup.sh` script using this variable set to `true`, by default, a new secret key is generated and distributed for both the {ControllerName} and the {CatalogName}.|`False`
|====
