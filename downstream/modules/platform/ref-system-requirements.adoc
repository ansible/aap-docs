...
* Ensure that the link:https://docs.redhat.com/en/documentation/red_hat_ansible_automation_platform/2.6/html/planning_your_installation/network-ports-protocols_planning#network-ports-protocols_table[Network ports and protocols] are available for successful connection and download of collections from {HubName} or {Galaxy} server. 
