...
[WARNING]
====
If you are compiling Postgres against OpenSSL 3.2, your system regresses to remove the parameter for User during startup. You can rectify this by using the `BIO_get_app_data` call instead of `open_get_data`. 
Only an administrator can make these changes, but it impacts all users connected to the PostgreSQL database.
