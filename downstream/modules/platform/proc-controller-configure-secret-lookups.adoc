...
[role="_abstract"]
{ControllerNameStart} can be configured to retrieve secrets from third-party secret management systems, such as HashiCorp Vault, AWS Secrets Manager, CyberArk Conjur, and others. 
...
. Create an external credential for authenticating with the secret management system. At a minimum, give a name for the external credential and select one of the following for the *Credential type* field:
+
