...
metadata:
    ...
    namespace: <namespace>
spec:
    deployment_name: <controller-instance-name>
----
+
Where:
...
[NOTE]
====
If you deployed {ControllerName} through an {PlatformNameShort} custom resource, the controller instance name follows the pattern `<aap-instance-name>-controller` unless you specified a custom name under `spec.controller.name` in your {PlatformNameShort} CR.

To find your controller instance name, run:
----
oc get automationcontroller -n <namespace>
----
====
...
----
+
This creates the `AutomationControllerMeshIngress` resource.
