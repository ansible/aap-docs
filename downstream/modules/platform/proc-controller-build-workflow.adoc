...
====
+
. If a job template used in the workflow has *Prompt on launch* selected for any of its parameters, a btn:[PROMPT] option is displayed, enabling you to change those values at the node level.
...
[NOTE]
====
If you hover over a step when adding a link and a red border is displayed, this means that you cannot connect those two steps together.
