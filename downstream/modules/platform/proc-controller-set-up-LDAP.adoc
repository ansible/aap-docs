...
----
+
. For more information on completing the mapping fields, *LDAP Organization Map* and *LDAP Team Map*, see the [ADD XREF]LDAP Organization and Team Mapping section.
. Click btn:[Save].

[NOTE]
====
{ControllerNameStart} does not actively sync users, but they are created during their initial login. To improve performance associated with LDAP authentication, see LDAP authentication performance tips at the end of this chapter.
====
