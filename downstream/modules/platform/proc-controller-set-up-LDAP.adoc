...
[NOTE]
====
If a user previously logged in using different case variations of their username, {PlatformNameShort} will now map all case variations to the lowercase username. Existing users with other case variations will no longer be valid for log in. A system administrator can delete those case variation users if needed.
====
