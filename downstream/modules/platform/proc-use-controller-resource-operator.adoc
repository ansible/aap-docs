...
. After installing the `automation-controller-resource-operator` in your cluster, you must create a Kubernetes (k8s) secret with the connection information for your {PlatformNameShort} instance.
. Then you can use {OperatorResourceShort} to create a k8s resource to manage your {PlatformNameShort} instance.