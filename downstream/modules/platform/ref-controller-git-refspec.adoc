...
* `refs/pull/62/head:refs/remotes/origin/pull/62/head`: This fetches the ref for that one GitHub pull request
...
A client can then launch the job template when a new pull request is created, providing the branch `pull/N/head` and the job template can run against the provided GitGub pull request reference.
