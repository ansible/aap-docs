...
[role="_abstract"]
{ControllerName} supports sourcing content for projects from Ansible collections defined in `requirements.yml` files. 
This is done by configuring {Galaxy} credentials at the organization level to define content sources for collection installations.
