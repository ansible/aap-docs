...
** *Reply URL (Assertion Consumer Service (ACS) URL):* This URL is auto generated when the SAML method is configured in {PlatformNameShort}. Copy that value from {PlatformNameShort} and paste in your IdP settings.
...
[NOTE]
====
References to the OneLogin SAML Python Toolkit are for formatting examples only. 
Some configuration entries in the OneLogin documentation might not apply to {PlatformNameShort}.
====
+
...
-----
+
For more information about the values you can include, see link:https://python-social-auth.readthedocs.io/en/latest/backends/saml.html#advanced-settings[advanced SAML settings].
+
[IMPORTANT]
====
Ensure to include all relevant values so that everything gets mapped correctly for your configuration. 
Alternatively, you can include the `GET_ALL_EXTRA_DATA: true` in the *Additional Authenticator Fields* to allow mapping of all available SAML IdP attributes.
====
+
