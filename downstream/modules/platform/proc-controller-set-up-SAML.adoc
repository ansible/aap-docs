...
* Configure a SAML Identify Provider (IdP).
...
. Include the private key content in the *SAML Service Provider Private Key*. This information is contained in the key.pem you created as a prerequisite and must include the `—–BEGIN PRIVATE KEY—–` and `—–END PRIVATE KEY—–`.
