...
You can configure {ControllerName} to talk with SAML to authenticate (create/login/logout) {ControllerName} users. 
...
This defaults to `true`. 
+
The `role` and `value` fields are lists and are _or_ logic. 
