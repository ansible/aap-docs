...
. Change the newly generated `inventory.new.ini` file to configure your {AutomationMesh} by assigning relevant variables, nodes, and relevant node-to-node peer connections:
+
[NOTE]
====
...
----
. Once you have finished configuring your `inventory.new.ini` for {AutomationMesh}, run the setup script by using `inventory.new.ini`:
+
----
$ ./setup.sh -i inventory.new.ini -e @credentials.yml -- --ask-vault-pass
----
. Once the installation completes, verify that your {PlatformNameShort} is successful by logging in to the {PlatformNameShort} dashboard UI across all {ControllerName} nodes.
