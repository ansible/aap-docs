...
If you have a subscriptions manifest, you can upload the manifest file by using the {PlatformName} interface. 
...
.Prerequisites
You must have a Red Hat subscription manifest file exported from the Red Hat Customer Portal. For more information, see link:https://docs.redhat.com/en/documentation/red_hat_ansible_automation_platform/2.6/html-single/containerized_installation/index#assembly-aap-obtain-manifest-files[Obtaining a manifest file].
...
Hosts automated:: Host count automated by the job, which uses the subscription count
