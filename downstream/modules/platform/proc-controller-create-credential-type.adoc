...
[role="_abstract"]
Define a custom credential type by using a YAML or JSON schema. 
Defining these custom types helps ensure that authentication information is securely injected into automation workflows.
...
* xref:controller-create-credential[Creating a credential]
