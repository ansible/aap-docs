...
| `automationedacontroller_verify_ssl`
| 
| Denotes whether or not to verify {EDAName}'s web certificates when making calls from {EDAName} to itself during installation.
Set to false to disable web certificate verification.
| Optional
| `true`
...
| Protocols that {EDAName} will support when handling HTTPS traffic.
| Optional
| `[TLSv1.2, TLSv1.3]`
...
|
| `eda_safe_plugins`
| List of plugins that are allowed to run within {EDAName}. For more information about the usage of this variable, see link:{URLContainerizedInstall}/ansible_automation_platform_containerized_installation#proc-add-eda-safe-plugin-var[Adding a safe plugin variable to {EDAcontroller}].
| Optional
| `[]`

|
| `eda_workers`
| Number of {EDAName} workers.
| Optional
|
