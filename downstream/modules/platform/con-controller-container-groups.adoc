...
{PlatformNameShort} supports container groups, which allow you to execute jobs in {ControllerName} regardless of whether {ControllerName} is installed as a standalone, in a virtual environment, or in a container. 
...
[NOTE]
====
Container groups upgraded from versions prior to {ControllerName} 4.0 revert back to default and completely remove the old pod definition, clearing out all custom pod definitions in the migration.
====
