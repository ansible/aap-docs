...
[role="_abstract"]
Use the `curl` command line tool to launch {ControllerName} jobs programmatically through the REST API. 
This method provides a direct way to integrate job execution into external scripts, ensuring immediate control.
...
Use caution when escaping the quotes.
====
