...
= Remove the retained Postgres 13 PVC

[role="_abstract"]

After upgrading {PlatformName} from a version that uses Postgres 13 to one that uses Postgres 15, the original Postgres 13 Persistent Volume Claim (PVC) is not automatically removed for {ControllerName} deployments using a managed database. If your deployment uses an external database, no PVC cleanup is required. You must manually delete the retained PVC after confirming that the upgrade completed successfully.
