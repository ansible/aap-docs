...
| Yes
| *Inventory* a| Choose the inventory to use with this job template from the inventories available to the logged in user.
...
| *Source control branch* a| This field is only present if you chose a project that allows branch override.
...
| *Playbook* a| Choose the playbook to be launched with this job template from the available playbooks.
...
You must select a project before you can select an {ExecEnvShort}. a| Yes

Execution environment prompt show up as its own step in a later prompt window.
| *Credentials* a| Select the image:examine.png[examine,15,15] icon to open a separate window.
...
Some credential types are not listed because they do not apply to certain job templates. a| - If selected, when launching a job template that has a default credential and supplying another credential replaces the default credential if it is the same type.
...
For guidance on using multiple credential types together, such as Machine and Network credentials, see xref:proc-controller-multi-cred-machine-network[Using Machine and Network credentials together].
...
- You cannot delete existing labels, selecting image:disassociate.png[Disassociate,11,11] only removes the newly added labels, not existing default labels.
...
//Commenting out as no downstream alternative yet[AAP-57435]
//For more information, see link:https://docs.ansible.com/projects/ansible/latest/inventory_guide/intro_patterns.html[Patterns: targeting hosts and groups] in the Ansible documentation. 
| Yes
...
| *Verbosity* a| Control the level of output Ansible produces as the playbook executes.
...
| *Instance groups* a| Choose xref:controller-instance-and-container-groups[Instance and Container Groups] to associate with this job template.
...
| *Job tags* a| Type and select the *Create* menu to specify which parts of the playbook should be executed.
//Commenting out as no downstream alternative yet [AAP-57435]
...
| *Skip tags* a| Type and select the *Create* menu to specify certain tasks or parts of the playbook to skip.
//Commenting out as no downstream alternative yet[AAP-57435]
//For more information and examples see link:https://docs.ansible.com/projects/ansible/latest/playbook_guide/playbooks_tags.html[Tags] in the Ansible documentation. 
| Yes
