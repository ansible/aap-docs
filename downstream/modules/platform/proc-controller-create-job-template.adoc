...
Tasks that do not support check mode will be skipped and will not report potential changes. 
...
| Credentials | Select the search icon to open a separate window. 
...
- Select "x" beside a label to remove it. 
...
- You cannot delete existing labels, selecting "x" only removes the newly added labels, not existing default labels.
| Variables a|
...
If the list is extensive, use the search icon to narrow the options.
...
* *Provisioning Callbacks*: If checked, you enable a host to call back to {ControllerName} via the REST API and invoke the launch of a job from this job template. 
