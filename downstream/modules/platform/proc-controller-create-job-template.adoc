...
- Credential prompts will show up as its own step in a later prompt window.
...
If selected, you are providing the jobs preferred instance groups in order of preference. If the first group is out of capacity, subsequent groups in the list are considered until one with capacity is available, at which point that will is selected to run the job.
...
** For additional information about setting up webhooks, see link:https://docs.ansible.com/automation-controller/latest/html/userguide/webhooks.html#ug-webhooks[Working with Webhooks].
