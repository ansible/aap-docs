...
.Procedure
. Create a secret in your {PlatformNameShort} namespace from the `bundle-ca.crt` file (the filename must be `bundle-ca.crt`):
+
----
...
+
----
oc exec -it deployment.apps/aap-gateway - openssl x509 -in /etc/pki/tls/certs/ca-bundle.crt -noout -text
----
