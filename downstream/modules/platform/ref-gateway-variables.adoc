...
| `gateway_admin_user` 
| Username used to identify and create the administrator user in {Gateway}. The installation program uses this account to register services with {Gateway}. If you have deleted the default `admin` user, set this variable to an existing system administrator account to avoid installation or upgrade failures.
| Optional
| `admin`
