...
Note that when restricting administrative access through mechanisms such as _sudo_, restricting to a certain set of commands can still give a wide range of access. 
Any command that enables execution of a shell or arbitrary shell prompt commands, or any command that can change files on the system, is equal to full root access.
