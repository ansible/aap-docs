...
* The `ansible.platform` collection replaces the service-specific `ansible.controller`, `ansible.hub`, and `ansible.eda` collections. For more information, see link:https://docs.redhat.com/en/documentation/red_hat_ansible_automation_platform/2.6/html/release_notes/aap-26#aap-2.6-deprecated-features[Deprecated features] in the _Release Notes_ for {PlatformNameShort} 2.6.
