...
* Access to _baseos_ and _{PlatformNameShort}stream_ repository packages for the RHEL 9 host.
* A non-root login account to the RHEL 9 host for installation.
** The login account requires passwordless `sudo` access to root.
...
# Ensure this host resolves to an IP address accessible by the installer. 
# If the public IP blocks port 5432, you must use the internal IP address.
