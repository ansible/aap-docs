...
* {AutomationDashboardNameStart} installation has been tested with the following configuration: 
...
. Install the required `ansible-core` package.
+
[source,bash]
----
cd ansible-automation-dashboard-containerized-setup
...
** If your {AutomationDashboardName} images are hosted in a different registry than your core components, you must provide separate login credentials using the `registry_<VARIABLE_NAME>_aap_automation_dashboard` variables.
...
----
# This is our {AutomationDashboardName} front-end application
[automationdashboard]
...
. Install the required Ansible collections. You must complete this step to prevent module resolution errors during the installation.
+
[source,bash]
----
