...
* Open firewall access to allow for bidirectional communication between AAP instances and the {AutomationDashboardName}. 
...
* Scope: read
... Store this access token value. `clusters.yaml` uses this access token.
+
