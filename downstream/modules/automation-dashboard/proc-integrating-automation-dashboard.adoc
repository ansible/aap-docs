...
. Verify that {AutomationDashboardName} is running on https port 8447 on your Red Hat Enterprise Linux host.
+
[NOTE]
====
...
====
. Add your {PlatformNameShort} instances into `clusters.yaml` using the following information:
...
[NOTE]
====
If you do not have access to {PlatformNameShort}, consult your administrator.
====
...
The utility uses the ping endpoint to detect the {PlatformNameShort} version and find the correct token refresh endpoint.
====
*Example*
+
[source,bash]
...
====
You can then use `journalctl` to check progress:
+
[source,bash]
----
