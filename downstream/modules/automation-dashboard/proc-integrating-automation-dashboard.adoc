...
= Configure {AutomationDashboardName}
...
. Create or update the `clusters.yaml` file. You must include the `access_token`, `refresh_token`, `client_id`, and `client_secret` to enable persistent authentication and automatic token rotation.
+
[source,yaml]
----
clusters:
