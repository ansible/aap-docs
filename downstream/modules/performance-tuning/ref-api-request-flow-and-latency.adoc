...
|Performance is dependent on the configuration, capacity, and health of the load balancer or {OCPShort} Route. Any timeout for the external load balancer must be greater than or equal to the `client_request_timeout` setting passed to the installation program. This layer is responsible for distributing traffic if there are multiple {Gateway} nodes/pods.
...
|Handles standard API requests forwarded by Nginx. These servers process requests, validate JWT tokens, run API operations, and frequently interact with the database.
...
For more information on the performance of different authentication methods, see link:https://docs.redhat.com/en/documentation/red_hat_ansible_automation_platform/{PlatformVers}/html/performance_tuning_for_ansible_automation_platform/api_service_performance#scaling-gateway-proxy-and-authentication_aap-api-performance[Considerations for scaling the {Gateway} proxy and authentication service].
