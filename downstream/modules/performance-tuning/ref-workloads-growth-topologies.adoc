...
|10 concurrent jobs with default forks (5 forks is default) + 100 with forks=1

|Callback receiver event processing rate
|10,000 job events per second at peak

|Job History with 30 days retention
|2kb event; 500 events per playbook run; 500 jobs a day + Less than 60Gb (as specified as minimum required disk on Database node)
