...
[role="_abstract"]
Review frequently asked questions about {Navigator} configuration and behavior. This information helps you troubleshoot common problems and optimize your environment setup.
...
Ansible looks for the `ansible.cfg` in the typical locations when not using an {ExecEnvNameSing}. 
See link:{URLNavigatorGuide}/assembly-review-config-navigator_installing-devtools[Reviewing your Ansible configuration with automation content navigator].
...
Where should Ansible collections be placed when not using an {ExecEnvNameSing}?:: When not using an {ExecEnvNameSing}, Ansible looks in the default locations for collections. See the link:{URLDevelopAutomationContent}/devtools-create-roles-collection_installing-devtools[Creating a collection for distributing roles] guide.
