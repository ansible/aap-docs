...
.Procedure
. Add a lifecycle block to call the EDA event stream to your `*.tf` file. The `after_create` action will trigger the `action.aap_eda_eventstream_post.create`.
+
**Example**
...
+
* **`event_stream_config`:** (Attributes) Details for the EDA Event Stream. You must include:
