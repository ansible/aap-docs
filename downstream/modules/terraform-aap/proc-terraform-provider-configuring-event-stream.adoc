...
= Configuring an EDA event stream

[role="_abstract"]

To use TF Actions with EDA, you must first configure the event stream in {PlatformNameShort}. TF actions will post events to this stream.
...
. Click **Create event stream**. Then navigate to **{MenuADEventStreams}** to verify the event stream was created and see the number of events received so far. 
+
...
. Click **{MenuADRulebookActivations}** to verify that the rulebook is active and check its status.
