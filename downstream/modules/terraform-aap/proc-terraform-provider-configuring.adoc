...
* **`password`:** (String, Case Sensitive) Password to use for basic authentication. Ignored if the token is set. Can also be configured by setting the `AAP_PASSWORD` environment variable.
...
* **`token`:** (String, Case Sensitive): Token to use for token authentication. Can also be configured by setting the `AAP_TOKEN` environment variable.
