...
[NOTE]
====
By default, the Terraform registry displays the latest version which might be an pre-release version (for example, 1.4.0-devpreview). Use the dropdown list to select a release version, which does not include a dash (for example, 1.4.0). Only release versions are supported.
====
