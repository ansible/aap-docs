...
Backups contain a copy of the database as well as the secret key used to decrypt credentials stored in the database. Store the backup files in a secure encrypted location. 
Otherwise, credentials stored in the backup file may be at risk of being disclosed. 
Limit access to backups only to {PlatformNameShort} administrators who have root shell access to the infrastructure nodes and the dedicated installation host.
