...
Since {PlatformNameShort} defaults to using the `ansible.builtin.sudo` link:https://docs.ansible.com/projects/ansible/latest/plugins/become.html[become plugin] on Linux-based managed nodes, the service account must be permitted to run any command on the RHEL managed node using the sudo command. 
