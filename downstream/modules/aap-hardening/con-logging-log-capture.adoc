...
{PlatformNameStart}'s built-in logging and activity stream support {PlatformName} logs all changes within {PlatformName} and automation logs for auditing purposes. 
...
{PlatformNameStart} must be configured to use external logging to compile log records from multiple components within the {PlatformNameShort} server. 
