...
Set `fapolicyd` to permissive mode on {PlatformNameShort} using the following steps:
...
[NOTE]
====
If this security control is also applied to the installation host, the default `fapolicyd` configuration causes the {PlatformNameShort} {Installer} to fail. 
In this case, the recommendation is to set `fapolicyd` to permissive mode on the installation host.
====
