...
[id="ref-install-user-pki_{context}"]
...
Where an existing PKI infrastructure is available, certificates must be generated for the {ControllerName}, {PrivateHubName}, {EDAcontroller}, and the PostgreSQL database server. 
...
To install unique certificates and keys on each {Gateway}, the certificate and key variables in the installation inventory file must be defined as host variables instead of in the `[all:vars]` section. 
For example:
+
----
[automationgateway]
