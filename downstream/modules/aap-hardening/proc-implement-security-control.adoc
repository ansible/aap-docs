...
=======
[role="_abstract"]
{PlatformNameShort} must use external log providers that can collect user activity logs in independent, protected repositories to prevent modification or repudiation. 
>>>>>>> 9211c0e73 (Revision for 5th iteration of Hardening Guide (#5638))
...
The following steps implement secure logging for {PlatformNameShort}:
...
* Set *Logging Aggregator* to the host name or IP address of the external log server.
...
. Click btn:[Save].
+
{PlatformNameShort} must allocate log record storage capacity and shut down by default upon log failure (unless availability is an overriding concern). 
