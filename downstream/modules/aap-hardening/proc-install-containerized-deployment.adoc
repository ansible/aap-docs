...
[role="_abstract"]
For containerized installations of {PlatformNameShort}, use the provided automation execution playbook with the external Ansible vault file.
...
.Procedure
. Ensure your Ansible vault file (for example, `vault.yml`) contains all required sensitive variables.
