...
[role="_abstract"]
The process to Update the plug-in registry requires you to login to {OCPShort}, initiate the build with `oc` start-build plugin-registry to deploy the new TAR files, and immediate verification within the plugin-registry pod terminal.
