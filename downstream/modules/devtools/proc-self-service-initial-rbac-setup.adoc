...
* **Auto-generated templates:** Permissions are synchronized from {PlatformNameShort}. Users must have permissions on the underlying {PlatformNameShort} job template.
* **Custom templates:** Permissions must be explicitly configured within the {SelfServiceShort}. Users must also have permissions to run the associated job templates in {PlatformNameShort}.

For help with RBAC on custom templates, see link:{BaseURL}/red_hat_ansible_automation_platform/{PlatformVers}/html-single/using_self-service_automation_portal/index#self-service-set-up-rbac_self-service-rbac[Setting up RBAC for custom self-service templates].
====
...
. Log in to {SelfServiceShort} as a non-admin user who has the assigned permissions or is a member of a team with the assigned permissions. 
