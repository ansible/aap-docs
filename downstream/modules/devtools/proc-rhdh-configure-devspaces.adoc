...
= Configure {DevSpacesShort}

[role="_abstract"]
When {DevSpacesShort} is configured for the Ansible plug-ins, users can click a link from the catalog item view in {RHDH} and edit their provisioned Ansible Git projects using Dev Spaces. 

[NOTE]
====
{DevSpacesShort} is a separate product and it is optional.
...
If the {DevSpacesShort} link is not configured in the Ansible plug-ins,
