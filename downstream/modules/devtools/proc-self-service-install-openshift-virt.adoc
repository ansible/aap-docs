...
+
The appliance requires configuration through cloud-init user-data, a pre-seeded configuration file, or post-deployment YAML editing.
For details, see xref:proc-self-service-initial-config-wizard_{context}[Configuring the appliance at first boot].

. Verify that portal services are running.
+
SSH into the appliance and check the service status:
+
[source,terminal]
----
