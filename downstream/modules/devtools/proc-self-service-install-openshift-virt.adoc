...
= Installing {SelfService} on OpenShift Virtualization

[role="_abstract"]

Deploy the {SelfService} appliance on OpenShift Virtualization to run the portal as a virtual machine alongside container workloads within your OpenShift Container Platform cluster. This approach enables you to use your existing OpenShift infrastructure to host the portal.
...
+
Follow the on-screen prompts in the console. For more information, see xref:proc-self-service-initial-config-wizard_{context}[Completing the initial configuration wizard].
