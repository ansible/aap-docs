...
// [learning path no longer exists--tutorial link should be replaced if a new link becomes available. -herbly] See link:https://red.ht/aap-lp-vscode-essentials[Learning path - Getting Started with the Ansible {VSCode} Extension] for tutorials on working with the extension.
