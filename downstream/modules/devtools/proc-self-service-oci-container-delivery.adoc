...
* You have a registry service account token from the Red Hat Customer Portal. For more information, see link:https://access.redhat.com/login?redirectTo=https%3A%2F%2Faccess.redhat.com%2Farticles%2FRegistryAuthentication[Registry Service Accounts].
...
[NOTE]
====
You must use a Red Hat Registry service account to generate the OpenShift secret.
Using your standard Red Hat account username and password  is not secure and does not work.
====
...
This is the default value. Omit this setting unless you must document the plugin delivery mode.
