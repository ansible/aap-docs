...
You can configure the Ansible VS Code extension to interact with models hosted on Red Hat AI infrastructure.
Supported platforms include the {RHEL} (RHEL) AI Inference Server and {OCPAI} (RHOAI).
This integration enables you to use the "Bring Your Own LLM" (BYOLLM) capability to generate and explain Ansible playbooks and roles.
You can use your own hosted models, such as Llama, Mistral, or OpenAI, without requiring external middleware.
...
. In Visual Studio Code, open the LLM provider settings:
+
.. In the Activity Bar, select the **Ansible** icon to open the Ansible Development Tools panel.
.. In the **GENERATIVE AI** section, select the settings icon (cog) to open the LLM provider configuration.
