...
[role="_abstract"]
To execute the installation use the `helm install` command to deploy the chart, referencing the local `.tgz` file directly and overriding critical values such as the namespace, the cluster router base, and the full, disconnected image path, using the required `--set` flags.
