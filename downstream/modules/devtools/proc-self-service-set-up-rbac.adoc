...
.Prerequisites
*  You have created a user, for example `example-user`. For more details, see link:{URLCentralAuth}/gw-managing-access#proc-controller-creating-a-user[Creating a user]. 
* You have added this user as a member of a team, for example `example-team`. Fore more details, see link:{URLCentralAuth}/gw-managing-access#proc-gw-team-add-user[Adding users to a team].
...
.. In the *Users and Groups* section, select the {PlatformNameShort} teams and users to assign to this role.
+
image::self-service-rbac-select-users-groups.png[Select users and groups table showing Members column]
+
[NOTE]
====
