...
[NOTE]
====
Hardening and compliance for {PlatformNameShort} 2.4 includes additional considerations with regards to the specific _Defense Security Information Agency_ (DISA) _Security Technical Implementation Guides_ (STIGs), but this guidance does not apply to {PlatformNameShort} {PlatformVers}.
====
...
Additional considerations with regards to the Defense Information Systems Agency (DISA) Security Technical Implementation Guides (STIGs) for {RHEL} are provided for those organizations that integrate the DISA STIGs as a part of their overall security strategy.
