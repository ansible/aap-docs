...
//hherbly: removing as this is specific to partners, not a general user audience 
...
You can use {HubNameMain} to distribute the relevant {CertifiedColl}s to your users by creating a requirements file or a synclist. We recommend using a requirements file to install collections to your {HubName}, as synclists can only be managed by users with platform administrator privileges. 

Before you can use a requirements file to install content, you must: 

. xref:tbd[Configure a remote repository in your local hub]
. xref:tbd [Obtain an automation hub API token]
. xref:tbd [Use the hub API token to configure automation hub as a content source]
