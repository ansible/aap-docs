...
Similar to the {ControllerName} and {HubName} components, the setup for {EDAcontroller} includes default settings for specific variables in the inventory files. However, you must install {ControllerName} before you populate the inventory file with {EDAName} variables. 

In addition, {EDAcontroller} must be installed on a separate server and cannot be installed on the same host as {HubName} and {ControllerName}.
