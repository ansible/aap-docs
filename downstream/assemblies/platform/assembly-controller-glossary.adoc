...
When executed, a distributed job slices each inventory into several "slice size" chunks, which are then used to run smaller job slices.
...
While they can be used in playbooks and templates as with variables, facts are things that are inferred, rather than set. 
...
For the convenience of users who are switching from other configuration management systems, the fact module also pulls in facts from the `ohai` and `facter` tools if they are installed, which are fact libraries from Chef and Puppet.
...
The default is a very conservative five forks, though if you have a high quantity of RAM, you can set this to a higher value, such as 50, for increased parallelism.
...
Handlers::
Handlers are similar to regular tasks in an Ansible Playbook (see *Tasks*), but are only run if the Task contains a "notify" directive and also indicates that it changed something. 
...
Host::
A system managed by {ControllerName}, which can include a physical, virtual, or cloud-based server, or other device (typically an operating system instance). 
...
Job Detail::
The history of running a particular job, including its output and success or failure status.
...
For more information, see link:{URLControllerUserGuide}/controller-job-templates[Job templates].
...
Metadata::
Information for locating a secret in the external system once authenticated.  
The user provides this information when linking an
...
| Hybrid | Nodes that run persistent services and run jobs.
...
An instance of a notification type (Email, Slack, or Webhook) with a name, description, and a defined configuration.
...
Assigning a role to a group of hosts (or a set of groups, or host patterns) implies that they implement a specific behavior. 
...
A set consisting of any combination of job templates, project syncs, and inventory syncs, linked together to run them as a
single unit.
