...
The following describe best practice for the use of {ControllerName}

== Use Source Control

Because {ControllerName} supports playbooks stored directly on the server, recommended best practice is to store your playbooks, roles, and any associated details in source control. 
...
[NOTE]
====
* Playbooks should not use the `vars_prompt` feature, as {ControllerName} does not interactively permit `vars_prompt` questions. 
If you cannot avoid using `vars_prompt`, see the xref:controller-surveys-in-job-templates[Surveys] functionality.

* Playbooks should not use the `pause` feature of Ansible without a timeout, as {ControllerName} does not permit cancelling a pause interactively.
