...
A metrics endpoint, `/api/v2/metrics/` is available in the API that surfaces instantaneous metrics about the controller, which can be
...
Prometheus can be configured to scrape these metrics from {ControllerName} by hitting the {ControllerName} metrics endpoint and storing this data in a time-series database. 
