...
If you have not installed {OperatorPlatform} see xref:install-aap-operator_operator-platform-doc[Installing the {PlatformName} operator on {OCP}].
...
If you have the {OperatorPlatform} and some or all of the {PlatformNameShort} components installed see xref:operator-deploy-central-config_{context}[Deploying the platform gateway with existing {PlatformNameShort} components] for how to proceed. 
