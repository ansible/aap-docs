...
For more information about the requirements file format, see the link:https://docs.ansible.com/projects/ansible/latest/galaxy/user_guide.html#install-multiple-collections-with-a-requirements-file[Galaxy User Guide].
| *python* | The Python installation requirements. 

This can be a filename, or a list of requirements. 
{Builder} combines all the Python requirements files from all collections into a single file by using the `requirements-parser` library. 
...
Entries from many collections that are duplicates of each other can be consolidated in the combined file.
|====
