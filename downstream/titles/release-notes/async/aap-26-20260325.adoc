...
* Namespace: aap-operator.v2.6.0-0.1774367109

* Cluster: aap-operator.v2.6.0-0.1774367079
...
This asynchronous update for Red Hat {PlatformNameShort} 2.6 (2.6.20260325) provides targeted enhancements, security updates, and bug fixes across {ControllerName}, {Gateway}, {HubName}, {EDAName}, Lightspeed, {ExecEnvShort}, {OperatorPlatformNameShort}, and both platform and containerized installers.
...
Addresses issues impacting UI responsiveness, containerized installer behavior after Django upgrades, constructed inventory and facts handling, credential validation in Event-Driven Ansible, database restore flows in platform operators, and certificate handling in execution environments.
...
This update rebases the containerized installer to ansible.platform collection version 2.6.20260306, aligning the installer with the current AAP 2.6 collection release. AAP-67548

== Features

=== {ControllerNameStart}

* This update improves compatibility with the receptor control tooling used by automation controller by updating the pinned receptorctl version for Tower 4.7 / AAP 2.6. AAP-66806
...
* This update prepares for future token management changes by adding a deprecation warning for the ah_token module in the ansible.hub collection on AAP 2.6 (Hub 4.11) behind AAP gateway. AAP-65109
...
* Fixed an issue where an organization administrator could not assign team access to projects in AAP 2.6, preventing proper delegation of permissions. AAP-65081
...
* Fixed an issue where Jinja2 variable substitution in rule names failed in EDA controller worker mode even though the same variables worked in action `extra_vars`, aligning behavior with the CLI. AAP-67038

* Fixed an issue where EDA server could not sync git projects using `ssh:// or git+ssh://` URL schemes, restoring project sync behavior. AAP-66353

=== {ExecEnvNameStartSing}

* Fixed an issue where a change in the `certifi` package affected default trust store paths in AAP 2.6 execution environments by switching to `system-certifi` to restore expected behavior. AAP-58769
