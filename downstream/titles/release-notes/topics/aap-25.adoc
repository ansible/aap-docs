...
You can only use new installations of {EDAName} in this configuration. RPM-based hybrid deployments are fully supported by Red Hat. See *Install and configure {EDAName} 1.1 with {ControllerName} 4.4 or 4.5* in link:https://docs.redhat.com/en/documentation/red_hat_ansible_automation_platform/2.4/html-single/using_event-driven_ansible_2.5_with_ansible_automation_platform_2.4/index[{EDAName} 2.5 with {ControllerName} 2.4] for details on setting up this configuration.
