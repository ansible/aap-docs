...
== General availability of Ansible {SelfServiceShort} 2.1.1

Ansible {SelfServiceShort} is now generally available as part of the {PlatformNameShort} subscription. The new {SelfServiceShort} empowers platform admins to provide a streamlined “point-and-click” Ansible automation experience to a broader set of users within the organization. Users who are not Ansible experts now have a dedicated self-service portal from which they can launch a range of automation jobs.

{SelfServiceShortStart} includes a set of usability improvements focused on reducing friction in common tasks.
...
* Installation: Deployment of {SelfServiceShort} requires {OCP} using a Helm chart. A future deployment of {SelfServiceShort} on {RHEL} 10 is planned for Technology Preview in a future asynchronous release of {PlatformNameShort} {PlatformVers}.
...
{AutomationDashboardNameStart} is now generally available as part of the {PlatformNameShort} subscription. {AutomationDashboardNameStart} is a utility you can connect to one or more {PlatformNameShort} deployments to visualize automation usage data, determine time savings, track ROI, and drive increased visibility into automation strategy, resource allocation, and prioritization of automation projects. Benefits include:
...
For more information, see link:https://docs.redhat.com/en/documentation/red_hat_ansible_automation_platform/2.6/html/using_automation_dashboard/index[Using {AutomationDashboardName}]. 
...
* link:https://docs.redhat.com/en/documentation/red_hat_ansible_automation_platform/{PlatformVers}/html-single/installing_on_openshift_container_platform/index#operator-upgrade_deploying-chatbot-operator[Upgrading {OperatorPlatformName} on {OCP}]
...
* **Authentication changes**: Enterprise authentication configuration and mappings, including SAML, LDAP, and OIDC, automatically move from {ControllerName} 2.4 to {Gateway} {PlatformVers} during the upgrade. 
Although these settings migrate automatically within {PlatformNameShort}, you must update the callback URLs in your external Identity Provider (IdP) settings to point to {Gateway}. For more information, see link:{URLPlanningUpgrade}/upgrade-authentication-movement-2.4-to-2.6[Authentication movement].
+
