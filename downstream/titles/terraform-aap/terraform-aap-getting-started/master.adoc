:imagesdir: images
:numbered:
:toclevels: 1
:experimental:

include::attributes/attributes.adoc[]

// Book Title
// This guide was formerly named Getting started with Terraform and Ansible Automation Platform [Jonquil Williams]
// Chuck - removing the attribute in the title b/c Pantheon is not resolving, at least on Preview branch
= Getting started with HashiCorp and Ansible Automation Platform

include::{Boilerplate}[]
include::terraform-aap/assembly-terraform-product.adoc[leveloffset=+1]
include::terraform-aap/assembly-terraform-introduction.adoc[leveloffset=+2]
include::terraform-aap/assembly-terraform-integrating-from-aap.adoc[leveloffset=+2]
include::terraform-aap/assembly-terraform-migrating.adoc[leveloffset=+2]
include::terraform-aap/assembly-terraform-integrating-from-terraform.adoc[leveloffset=+2]
include::vault-aap/assembly-vault-product.adoc[leveloffset=+1]
include::vault-aap/assembly-vault-introduction.adoc[leveloffset=+2]
include::vault-aap/assembly-vault-authenticating.adoc[leveloffset=+2]
include::vault-aap/assembly-vault-migrating-from-community.adoc[leveloffset=+2]
