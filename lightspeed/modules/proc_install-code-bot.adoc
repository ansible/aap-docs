...
. Optional: If required, you can xref:manually-scan-repo_using-code-bot-for-suggestions[manually scan your repositories] or xref:configure-repo-scan_configuring-repository-scans[change the scan schedule to a daily or monthly cadence]. 
. Modify scanned repositories by xref:add-remove-repo-from-scans_configuring-repository-scans[adding repositories or removing existing repositories from being scanned].
