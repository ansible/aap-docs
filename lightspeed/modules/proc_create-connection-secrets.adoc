...
You must create authorization secret to connect to {PlatformName}, and a model secret to connect to {ibmwatsonxcodeassistant}. If you need to trust a custom Certificate Authority, you must create a bundle secret.
...
After you create the authorization and model secrets, you must select the secrets when you  xref:create-lightspeed-instance_configuring-lightspeed-onpremise[create and deploy an {LightspeedshortName} instance].
...
.Procedure
Use one of the following methods to create a custom bundle secret using CLI: 
