...
[role="_abstract"]
After the {AnsibleContentParser} scans your Ansible files, it generates the training data set in an output directory. The training data set includes a *ftdata.jsonl* file, which is the main output of the {AnsibleContentParser}. 

The file is available in JSON Lines file format, where each line entry represents a JSON object. You must upload this JSONL file to {ibmwatsonxcodeassistant} to create a custom model. 
