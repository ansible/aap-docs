...
[role="_abstract"]
Review the {Azure} infrastructure components, resource specifications, and database configurations deployed with {PlatformNameShort} to estimate usage, plan capacity, and manage scaling.

When you install {AAPonAzureNameShort}, you deploy the following infrastructure into your {Azure} subscription:

Managed identity:: Used by {PlatformNameShort} to authenticate with Azure services. Ensure your subscription allows managed identity creation.
...
[IMPORTANT]
====
Before upgrading to {PlatformNameShort} {PlatformVers}, ensure your Azure quota and budget account for the mandatory transition to the AKS Standard tier. 
====
+
**Service Shape for all {PlatformNameShort} plan sizes:**
+
Select your database shape based on your expected {PlatformNameShort} node count:
+
