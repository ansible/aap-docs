...
[id="con-saas-backup-and-disaster-recovery"]
...
* The backup and restore disaster recovery (DR) model will be deprecated on August 3, 2026, for environments exceeding 50 nodes. Customers currently using this model must migrate to the new multi-region active-passive infrastructure shape before August 3, 2027. For more information. see link:https://docs.redhat.com/en/documentation/ansible_on_clouds/2.x/html/red_hat_ansible_automation_platform_service_on_aws/saas-service-definition#proc-saas-existing-customers[Existing customers].
====
