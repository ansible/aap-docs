...
During initial deployment, customers can configure networking address range (CIDR block) for the VNet the {PlatformNameShort} uses. After configuration, when you have created the network, you cannot change CIDR blocks. To make a change you must redeploy the managed {PlatformNameShort} application with the configurations specified in the documentation.
Failure to remediate:::
