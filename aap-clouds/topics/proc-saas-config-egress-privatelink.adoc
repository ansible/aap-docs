...
=  Configure {AWSPrivateLink} connectivity from Red Hat managed control plane to customer VPCs

[role="_abstract"]
Configure AWS PrivateLink egress connectivity by creating an Endpoint Service, submitting a support request to Red Hat, and updating DNS to enable the {PlatformNameShort} control plane to access your private resources.
...
+
Copy the resulting VPC Endpoint Service Name from your Endpoint Service configuration into the following *Egress PrivateLink request template*, fill in the remaining details, and submit a *separate* {CustomerSupport} case to Red Hat.
+
----
Subject: 
