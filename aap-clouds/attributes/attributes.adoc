...
// AAP on Google Cloud is deprecated
:GCPMarket: GC Marketplace
:GCP: Google Cloud 
:AAPonAWS: Ansible Automation Platform from AWS Marketplace
:AAPonGCP: Ansible Automation Platform from GC Marketplace
...
:MenuSetAutomationExecution: menu:{MenuAEAdminSettings}[Automation Execution] 
//[Jameria] Note to Aine: I added the Settings menu above because it seems there has been a change in the UI. Now, the following menu selections that used to reside under Settings have been nested under the Automation Execution selection. So the following need to be updated. Since you have handled these before, I wanted to bring it to your attention in the attributes file. Going to post a message for you in our Slack channel, #ccs-aap-docs. 
