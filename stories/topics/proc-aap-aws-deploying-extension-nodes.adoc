...
The upgrade process requires a few volumes to be mounted, so *prepare a new directory*.
...
Run the upgrade playbook as a container. Replace `<deployment-name>` with your foundation deployment name. Use the following command to run the playbook.
