...
.Procedure
* Pull the Docker image for Ansible on Clouds operational container with the tag based on the version you want to upgrade from. If you are unsure of the version you have deployed and the one you need to upgrade to, see xref:ref-aap-using-playbooks[Command Generator] and playbook *gcp_get_aoc_version* for more information on figuring out the current version of Ansible on Clouds deployment and the version to upgrade from.
+
[NOTE]
====
