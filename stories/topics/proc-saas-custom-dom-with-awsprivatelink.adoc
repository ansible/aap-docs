...
+
There are a few entries. To find the correct DNS name, go to the *Details* tab and look for the entry that isn't tied to a specific Availability Zone (AZ). For instance, choose `vpce-xxxx-xxxx.vpce-svc-xxxx.us-east-1.vpce.amazonaws.com` rather than one that includes an AZ like `us-east-1a` or `us-east-1b`.
+
